...
ZELDA_DIR = os.path.join(BASE_DIR, "..", "zelda", "BS Zelda")   # 원본 맵 폴더
OUTPUT_DIR = os.path.join(BASE_DIR, "..", "processed_BS")  # 잘라낸 방 저장 폴더
...
# 3. 단일 이미지 처리
# =========================
TARGET_FILE = "BSZelda(J)-Hyrule.png"  # 👉 여기 파일 이름만 바꿔주면 됨
img_path = os.path.join(ZELDA_DIR, TARGET_FILE)

count = slice_map(img_path, rows=7, cols=8)  # 👉 원하는 방 개수로 설정
